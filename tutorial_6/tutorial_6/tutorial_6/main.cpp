...
void opencl_setup_scratch() {
	...
	//err = clGetDeviceIDs(NULL, CL_DEVICE_TYPE_CPU, 1, &cdDevices[1], &nd);

	//m_context = clCreateContextFromType(0, CL_DEVICE_TYPE_CPU, NULL, NULL, NULL);
	...
	//queue_cpu = clCreateCommandQueue(m_contextcpu, cdDevices[1], 0, NULL);
	queue_gpu = clCreateCommandQueue(m_context, cdDevices[0], 0, NULL);

	//bufferRandom = clCreateBuffer(m_context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
		//sizeof(cl_int) * HEIGHT * WIDTH, random, NULL);
	//dbuffer_matrix2 = clCreateBuffer(m_context, CL_MEM_WRITE_ONLY, sizeof(cl_int) * HEIGHT * WIDTH,
	//	NULL, NULL);
	program = clCreateProgramWithSource(m_context, SOURCE_NUM_LINES, cgenerate_next_matrix,
		NULL, &err);
	//programCpu = clCreateProgramWithSource(m_context, SOURCE_NUM_LINEScpu, cgenerate_random,
	//	NULL, &err);
	cout << err;
	...
	szLocalWorkSize[0] = 1;
	//err = clEnqueueNDRangeKernel(queue_gpu, generate_random, 1, NULL, szGlobalWorkSize,
		//szLocalWorkSize, 0, NULL, NULL);
	cout << err << "heleloo";

	//err = clEnqueueReadBuffer(queue_gpu, dbuffer_matrix2, CL_TRUE, 0, HEIGHT * WIDTH * sizeof(cl_int),
		//random, 0, NULL, NULL);
	cout << err;
	//clFinish(queue_gpu);
